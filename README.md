...
| Arrangement A2B temporal/combined generation | NEXT | arpeggiation timing + combined transforms + broader corpus |
...
The most important case starts with an **8-note piano source sonority**. Strict realization fails because only six guitar strings exist, but source truth remains all eight notes. Under explicit reduction policy A2 emits 15 six-note alternatives; in the pinned benchmark all 15 are physically feasible. The required evidence candidate keeps `e1..e6`, remains source-complete in provenance, and is review-required.
...
- A2 has `automaticProductionTransformationAuthority = false`.
...
- Arpeggiation is representable by A1 but not automatically generated by the current A2 static slice because temporal revalidation is not implemented yet.
...
V3 exact + left-hand physical evidence
      ...
      +--> strict infeasible / explicit arrangement request
            ...
            A1 explicit N-best contract
                    |
                    v
            A2 bounded generation
                    |
                    v
       physical revalidation of every candidate
                    |
                    v
       A2B temporal / combined alternatives
                    |
                    v
...
- **Arrangement A2B temporal validation + combined transforms + broader piano corpus — NEXT**
